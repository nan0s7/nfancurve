...
sleep_override=""; gpu_cmd="nvidia-settings"
...
echo_info() {
	...
	e="$e mint=$mnt fan=$fan"
	prf "$e"
}

compare_spd() {
	if [ "$cur_t" -gt "$mnt" ]; then
		i=0
		for temp in $tc; do
...
set_stuff() {
	arr="$fan2gpu"; n="$fan"; re_elem
	gpu="$elem"
	arr="$which_curve"; n="$fan"; re_elem
	tmp="$elem"
	i=0
